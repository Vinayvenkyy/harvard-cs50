...
def index():
    ...
    for stock in stocks:
        total = total + (stock["price"] * stock["total_shares"])
...
def buy():
    ...
    if request.method == "POST":
        ...
        if int(shares) <= 0:
            return apology("Please enter a positve number of shares.")
        ...
        total_price= item_price * float(shares)
        new_cash= cash - total_price
        
        if cash < total_price:
            return apology("OOPS! Not enough cash")
        else:
...
def quote():
    ...
    return render_template("quoted.html", item = item, symbol = symbol)
...
def sell():
    ...
    if request.method == "POST":
        ...
        new_cash= current_cash + sold_money
